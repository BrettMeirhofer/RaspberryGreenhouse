...
def take_picture():
    ...
    for camera in config_dict["cams"]:
        for x in range(3):
            ...
            command = 'ffmpeg -f v4l2 -video_size 1280x720 -i {} -frames 1 -vf "vflip,hflip" {}'.format(camera["sys"], target_path)
            os.system(command)
            time.sleep(1)
        time.sleep(1)
        SendData.send_image_data(open(target_path, "rb"), params={"id": camera["id"]})
